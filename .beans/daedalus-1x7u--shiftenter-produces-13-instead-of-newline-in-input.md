...
updated_at: 2026-01-29T17:37:57Z
---
...
- [ ] Research: Confirm escape sequences in common terminals (Ghostty, iTerm2, Kitty, VS Code integrated terminal)
- [ ] Create `src/cli/raw-input.ts` module for raw mode input handling
- [ ] Implement escape sequence detection for Shift+Enter variants
- [ ] Handle normal keypress events (typing, backspace, arrows)
- [ ] Support existing features (history navigation, completion)
- [ ] Replace `question()` in `src/cli/plan.ts` with new raw input handler
- [ ] Test in multiple terminals (at minimum: iTerm2, VS Code terminal, Ghostty if available)
- [ ] Add fallback for terminals that can't be detected