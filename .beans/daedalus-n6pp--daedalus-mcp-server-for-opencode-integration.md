...
updated_at: 2026-01-31T04:44:26Z
---
...
- **Standalone entry point** — Add a `bin` entry in `package.json` so the MCP server can be used independently outside of OpenCode (e.g., with Claude Desktop or Zed)

## Scope

This is a larger feature. Consider breaking into child beans if it grows:
1. **MCP server core** — `src/mcp/server.ts` + `src/mcp/tools.ts` with stdio transport and `consult_experts` tool
2. **OpenCode integration** — Config generation and provider system prompt updates
3. **Standalone entry point** — `bin` entry in `package.json` for use with other MCP clients

For now, the checklist below covers the full scope as a single bean.
...
- `package.json` — Add `@modelcontextprotocol/sdk` dependency + optional `bin` entry
...
- [ ] Add `bin` entry in `package.json` for standalone MCP server usage
